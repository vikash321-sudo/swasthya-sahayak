...
// ─── SERVICES HUB ──────────────────────────────────────────────
function ServicesScreen({ setTab, lang, user }) {
      ...
      <div style={{ padding: "20px 16px 100px" }}>
        ...
        >
          ...
          {selectedService.id === "lab" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {[
                lang === "en" ? "Choose a test package" : "टेस्ट package छान्नुहोस्",
                lang === "en" ? "Select home sample collection time" : "घरमै sample collection समय छान्नुहोस्",
                lang === "en" ? "Lab partner confirms your booking" : "Lab partner ले booking confirm गर्छ",
                lang === "en" ? "Report appears inside Health Records" : "Report Health Records मा देखिन्छ"
              ].map((step, index) => (
          ...
          {selectedService.id === "medicine" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {[
                lang === "en" ? "Upload prescription or type medicine name" : "Prescription upload गर्नुहोस् वा औषधिको नाम लेख्नुहोस्",
                lang === "en" ? "Choose delivery or pickup" : "Delivery वा pickup छान्नुहोस्",
                lang === "en" ? "Partner pharmacy checks availability" : "Partner pharmacy ले availability check गर्छ",
                lang === "en" ? "User confirms order after pharmacy approval" : "Pharmacy approval पछि user ले order confirm गर्छ"
              ].map((step, index) => (
                <ServiceStep key={step} number={index + 1} text={step} />
              ))}
            </div>
          ...
          {selectedService.id === "nearby" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {[
                lang === "en" ? "Choose care type: dental, eye, child, emergency, pharmacy or lab" : "Care type छान्नुहोस्: dental, eye, child, emergency, pharmacy वा lab",
                lang === "en" ? "Use your location or saved district/local level" : "तपाईंको location वा saved district/local level प्रयोग हुन्छ",
                lang === "en" ? "Show nearby verified providers" : "नजिकका verified providers देखाइन्छ",
                lang === "en" ? "Call, request appointment or get directions" : "Call, appointment request वा direction लिन सकिन्छ"
              ].map((step, index) => (
                <ServiceStep key={step} number={index + 1} text={step} />
              ))}
            </div>
...
function ServiceStep({ number, text }) {
  return (
    <div
      style={{
        display: "flex",
